...
const u32 sTrainerFrontPic_LeafBig[] = INCGFX_U32("graphics/trainers/front_pics/leaf_big.png", ".4bpp.big.lz");
const u16 sTrainerPalette_LeafBig[] = INCGFX_U16("graphics/trainers/front_pics/leaf_big.png", ".gbapal");

EWRAM_DATA struct Even_BigSprite sBigSprite;

void stupidTask(u8 taskId)
{
    Even_MoveBigSprite(&sBigSprite, 1, 0);
}

void DrawBigLeaf(void)
{
    u32 tileTag = 0xCEC1;
    struct Even_BigSpriteSheet bigSheet;
    bigSheet.palette = sTrainerPalette_LeafBig;
    bigSheet.sprite = sTrainerFrontPic_LeafBig;
    bigSheet.tileTag = tileTag;
    bigSheet.palTag = tileTag;
    bigSheet.posX = 120;
    bigSheet.posY = 80;
    bigSheet.compressed = TRUE;
    bigSheet.subpriority = 0;

    Even_CreateBigSprite(&bigSheet, &sBigSprite);
}
