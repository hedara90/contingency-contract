...
bool32 AreMovesEquivalent(enum BattlerId battlerAtk, enum BattlerId battlerAtkPartner, enum Move move, enum Move partnerMove)
{
    ...
    if (move == MOVE_ROTOTILLER && partnerMove == MOVE_ROTOTILLER)
        return TRUE;
...
static enum AIScore IncreaseRototillerStatUpScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, bool32 isUser, enum Stat statId, s32 stages)
{
    enum AIScore tempScore = NO_INCREASE;
    u32 noOfHitsToFaint = NoOfHitsForTargetToFaintBattler(battlerDef, battlerAtk, AI_SHOULD_SETUP_DEFENDING, DONT_CONSIDER_ENDURE);
    enum Move predictedMove = GetPredictedMove(battlerAtk, battlerDef, gAiLogicData);
    bool32 aiIsFaster = AI_IsFaster(battlerAtk, battlerDef, MOVE_NONE, predictedMove, DONT_CONSIDER_PRIORITY); // Don't care about the priority of our setup move, care about outspeeding otherwise
    bool32 shouldSetUp = (!isUser || (noOfHitsToFaint >= 2 && aiIsFaster) || (noOfHitsToFaint >= 3 && !aiIsFaster) || noOfHitsToFaint == UNKNOWN_NO_OF_HITS);
    ...
    // Don't increase stats if player has a move that can change the KO threshold
    if (!isUser && aiIsFaster && HasMoveThatChangesKOThreshold(battlerDef, noOfHitsToFaint))
        return NO_INCREASE;
...
s32 GetRototillerScore(enum BattlerId battler, bool32 isUser)
{
    s32 tempScore = 0;
    if (IS_BATTLER_OF_TYPE(battler, TYPE_GRASS))
    {
        tempScore += IncreaseRototillerStatUpScore(battler, GetOppositeBattler(battler), isUser, STAT_ATK, 1);
        tempScore += IncreaseRototillerStatUpScore(battler, GetOppositeBattler(battler), isUser, STAT_SPATK, 1);
    }
    if (IS_BATTLER_OF_TYPE(battler, TYPE_GROUND))
    {
        tempScore += IncreaseRototillerStatUpScore(battler, GetOppositeBattler(battler), isUser, STAT_DEF, 1);
        tempScore += IncreaseRototillerStatUpScore(battler, GetOppositeBattler(battler), isUser, STAT_SPDEF, 1);
    }
    return tempScore;
}
