...
u32 AbilityBattleEffects(enum AbilityEffect caseID, enum BattlerId battler, enum Ability ability, enum Move move, bool32 shouldAbilityTrigger)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_HADRON_ENGINE:
            ...
            else if (TryChangeBattleTerrain(battler, STATUS_FIELD_ELECTRIC_TERRAIN, FALSE))
            {
...
static uq4_12_t GetWeatherDamageModifier(struct DamageContext *ctx)
{
    ...
    if (GetMoveEffect(ctx->move) == EFFECT_HYDRO_STEAM && (attackerWeather & B_WEATHER_SUN))
        return UQ_4_12(1.5);
    if (GetMoveEffect(ctx->move) == EFFECT_SAND_BLAST && (attackerWeather & B_WEATHER_SANDSTORM))
        return UQ_4_12(1.5);
...
s32 CalcCritChanceStage(struct DamageContext *ctx)
{
    ...
    if (critChance != CRITICAL_HIT_BLOCKED && ((!IsOnPlayerSide(ctx->battlerDef) && IsRiskActive(RISK_HAS_BATTLE_ARMOR)) || ctx->abilities[ctx->battlerDef] == ABILITY_BATTLE_ARMOR || ctx->abilities[ctx->battlerDef] == ABILITY_SHELL_ARMOR))
    {
...
void TryRestoreHeldItems(void)
{
    ...
    */
    ...
    for (u32 i = 0; i < PARTY_SIZE; i++)
    {
        if (gOriginalItems[i] != ITEM_NONE)
            SetMonData(&gParties[0][i], MON_DATA_HELD_ITEM, &gOriginalItems[i]);
    }
}
